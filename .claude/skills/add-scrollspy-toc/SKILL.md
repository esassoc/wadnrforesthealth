...
## When to Use ScrollSpyStickyDirective

For pages with complex layouts where CSS `position: sticky` alone isn't sufficient (e.g., content that can be longer than the viewport), add `ScrollSpyStickyDirective`:

```typescript
imports: [..., ScrollSpyStickyDirective]
```

```html
<div class="toc-sidebar" appScrollSpySticky=".content-area">
```
...
**For hierarchical with expand/collapse:**
```html
<nav class="toc-nav">
    @for (section of sections; track section.ID) {
    <div class="toc-item" [class.expanded]="isSectionExpanded(section)">
        <a href="javascript:void(0)"
           [class.active]="activeParentSectionId() === 'section-' + section.ID"
           (click)="scrollToSection('section-' + section.ID)">
            {{ section.Title }}
        </a>
        @if (section.ChildSections?.length > 0 && isSectionExpanded(section)) {
        <div class="toc-children">
            @for (child of section.ChildSections; track child.ID) {
            <a href="javascript:void(0)"
               [class.active]="activeSectionId() === 'section-' + child.ID"
               (click)="scrollToSection('section-' + child.ID, 'section-' + section.ID)"
               class="toc-child">
                {{ child.Title }}
            </a>
            }
...
- **Flat sections**: `LtInfo.Web/src/app/areas/threshold/about.component.ts` (before CMS conversion)
- **Hierarchical sections**: `LtInfo.Web/src/app/areas/threshold/threshold-evaluation-period-detail/`
- **With ScrollSpyStickyDirective**: `LtInfo.Web/src/app/areas/climate/outcome-detail/`
