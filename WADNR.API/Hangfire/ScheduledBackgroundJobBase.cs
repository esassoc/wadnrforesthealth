...
public abstract class ScheduledBackgroundJobBase<T>
{
    ...
    protected ScheduledBackgroundJobBase(string jobName, ILogger<T> logger, IWebHostEnvironment webHostEnvironment, WADNRDbContext dbContext, IOptions<WADNRConfiguration> ltInfoConfiguration, SitkaSmtpClientService sitkaSmtpClient)
    {
        _jobName = jobName;
        Logger = logger;
    ...
    public void RunJob(IJobCancellationToken token)
    {
        lock (ScheduledBackgroundJobLock)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Subject = $"WADNR Hangfire Job Failed: Job {_jobName}",
