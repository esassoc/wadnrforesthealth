...
namespace WADNRForestHealthTracker.Web
{
    public class Startup
    {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory, IHostApplicationLifetime applicationLifetime)
        {
            ...
            {
                context.Response.Headers["X-Ltinfo-Spa-Instance"] = "qa-spa-simple-fallback-v2";
                await next();
            });

            // 1. Special-case config.json
            app.Use(async (context, next) =>
            {
            ...
            app.Use(async (context, next) =>
            {
                ...
                var staticFileExtensions = new[]
                {
                    ...
                    ".ico", ".svg", ".map", ".json", ".woff", ".woff2",
                    ".ttf", ".eot"
                };

                var ext = Path.GetExtension(path);
                var isStaticFileRequest = !string.IsNullOrEmpty(ext) &&
                    staticFileExtensions.Contains(ext, StringComparer.OrdinalIgnoreCase);
