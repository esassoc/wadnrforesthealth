...
using System.Xml;
using WADNRForestHealthTracker.Common.DesignByContract;
    ...
    public class FileUtility
    {
        ...
        public static DirectoryInfo FirstMatchingDirectoryUpDirectoryTree(DirectoryInfo startingDir, string directoryFullNameRelativePath)
        {
            ...
            while (true)
            {
                ...
                {
                    throw new DirectoryNotFoundException($"Searched directory \"{startingDir.FullName}\" and upwards and could not find directory \"{directoryFullNameRelativePath}\".");
                }
            }
        }
        ...
        public static FileInfo FirstMatchingFileUpDirectoryTree(DirectoryInfo startingDir, string fileFullNameRelativePath)
        {
            ...
            while (true)
            {
                ...
                if (currentDirectory == null)
                {
                    throw new FileNotFoundException($"Searched directory \"{startingDir.FullName}\" and upwards and could not find file \"{fileFullNameRelativePath}\".");
                }
            }
        }
        ...
        {
            return (fileInfo.Attributes & FileAttributes.Hidden) == FileAttributes.Hidden || (fileInfo.Attributes & FileAttributes.System) == FileAttributes.System;
        }
        ...
        public static ZipArchive ExtractZipFile(Stream inputStream)
        {
            try
            {
                var zipArchive = new ZipArchive(inputStream, ZipArchiveMode.Read, true);
                return zipArchive;
            }
            catch (Exception ex)
            {                
                throw new InvalidDataException("Not a zip file.  Cannot extract contents.  " +ex.Message);
            }
        }

        public static DirectoryInfo ConvertZipArchiveToDirectoryInfo(ZipArchive zipArchive)
        {
            using (zipArchive)
            {
                return ConvertZipArchiveToDirectoryInfoImpl(zipArchive);
            }
        }

        public static DirectoryInfo ConvertZipArchiveToDirectoryInfoImpl(ZipArchive archive)
        {
            var dt = new DisposableTempDirectory();
            var dirToUnzipTo = dt.DirectoryInfo.FullName;
            ...
            foreach (var archiveEntry in archive.Entries)
            {
                if (!string.IsNullOrWhiteSpace(archiveEntry.Name))
                {
                    ...
                    //Console.WriteLine("File Name: {0}", archiveEntry.Name);
                    //Console.WriteLine("File Size: {0} bytes", archiveEntry.Length);
                    //Console.WriteLine("Compression Ratio: {0:0.0%}", ((double) archiveEntry.CompressedLength/archiveEntry.Length));

                    //Identifies the destination file name and path
                    var fileUnzipFullName = Path.Combine(dirToUnzipTo, archiveEntry.FullName);
                    ...
                    if (!System.IO.File.Exists(fileUnzipFullName))
                    {
                        ...
                        archiveEntry.ExtractToFile(fileUnzipFullName);
                    }
                }
                else
                {
                    //Creates the directory (if it doesn't exist) for the new path
                    var directoryPath = Path.Combine(dirToUnzipTo, archiveEntry.FullName);
            ...
            var dirLength = dt.DirectoryInfo.GetDirectories().Length;
            Check.Require(dirLength == 1, "We should only have one main folder in a zip archive. Found " + dirLength);
            return dt.DirectoryInfo.GetDirectories().First();
        }

        public static XmlDocument GetXmlFromZipArchive(ZipArchive zipArchive, string path)
        {
            ...
            {
                if (archiveEntry.FullName.Contains(path, StringComparison.InvariantCultureIgnoreCase))
                {
        ...
        public static String GetTextFileFromZipArchive(ZipArchive zipArchive, String modelName, String folderName, String fileNameWithExtension)
        {
            var doc = String.Empty;
            //Loops through each file in the zip file
            foreach (var archiveEntry in zipArchive.Entries)
            {
                if (archiveEntry.FullName.Contains($"{modelName}/{folderName}/{fileNameWithExtension}", StringComparison.InvariantCultureIgnoreCase))
                {
                    using (var stream = archiveEntry.Open())
