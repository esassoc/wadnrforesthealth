...
import { EIPProjectService } from "src/app/shared/generated/api/eip-project.service";
...
export class ProjectsMapComponent implements OnInit, OnChanges, OnDestroy {
    ...
    // cluster wrapper for the regional projects layer
    public regionalProjectsLayerClustered: any = null;
    // optional regional projects overlay (GeoJSON features)
    @Input() regionalProjects: IFeature[] | null = null;
    // private BehaviorSubject stores latest filtered list; expose as Observable for consumers
    private _regionalFilteredSubject: BehaviorSubject<IFeature[]> = new BehaviorSubject<IFeature[]>([]);
    public regionalProjectsFiltered$: Observable<IFeature[]> = this._regionalFilteredSubject.asObservable();
    public regionalLayer: L.Layer | null = null;
    ...
    private _haveInitialRegionalFlag: boolean = false;
    ...
    constructor(private eipProjectService: EIPProjectService) {}
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        if (this.map) {
            ...
            if (changes["regionalProjects"] && typeof this.regionalProjects !== "undefined") {
                this._haveInitialRegionalFlag = true;
            }

            // Use debounced refresh so rapid changes only cause a single update
            if (
                changes["projectPoints"] ||
                changes["colorByPropertyName"] ||
                changes["filterPropertyName"] ||
                changes["filterPropertyValues"] ||
                changes["cluster"] ||
                changes["regionalProjects"]
            ) {
                this.ensureRefreshSubscription();
                this._refresh$.next();
            }
    ...
    private ensureRefreshSubscription() {
        if (!this._refreshSub) {
            this._refreshSub = this._refresh$.pipe(debounceTime(250)).subscribe(() => {
                ...
                if (!this._haveInitialRegionalFlag && typeof this.regionalProjects !== "undefined") {
                    // treat an explicit null/array as 'arrived' for regional (it's optional)
                    this._haveInitialRegionalFlag = true;
                }
                ...
                if (!this._initDone) {
                    ...
                    this.debugLog("debounced - waiting for map+data before initial update", {
                        ...
                        haveRegional: this._haveInitialRegionalFlag,
                    });
                    return;
                }
    ...
    private updateRegionalLayer(): void {
        // compute filtered regional features using the same filter function used for the geoJSON layer
        const allRegional = this.regionalProjects || [];
        let filteredRegional: IFeature[] = allRegional;
        try {
            if (this.projectPointGeoJsonLayerOptions && typeof this.projectPointGeoJsonLayerOptions.filter === "function") {
                filteredRegional = (allRegional || []).filter((f: any) => {
                    try {
                        return this.projectPointGeoJsonLayerOptions.filter(f);
                    } catch (e) {
                        return true;
                    }
                });
            ...
            // swallow errors and fallback to full list
            filteredRegional = allRegional;
        }

        // emit the filtered regional list for parent components to consume
        try {
            // publish latest filtered list (BehaviorSubject will cache the last value)
            try {
                this._regionalFilteredSubject.next(filteredRegional || []);
            } catch (e) {}
        } catch (e) {}

        const res = this.updateLayerGeneric({
            features: allRegional,
            currentPlainLayer: this.regionalLayer,
            currentClustered: this.regionalProjectsLayerClustered,
            geoJsonOptions: this.projectPointGeoJsonLayerOptions,
            overlayName: "Regional (Approximate Location)",
            overlayGroup: "Projects",
            registerOnly: true,
            visibleWhenClustered: false,
        });

        this.regionalLayer = res.plainLayer;
        this.regionalProjectsLayerClustered = res.clustered;
    }
