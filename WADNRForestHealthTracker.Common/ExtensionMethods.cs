...
namespace WADNRForestHealthTracker.Common
{
    public static class ExtensionMethods
    {
        ...
        {
            Check.Require(maximumLengthAllowed >= 0, $"Need a positive maximum length, got {maximumLengthAllowed}");
            truncationMarker = truncationMarker ?? String.Empty;
            Check.Require(maximumLengthAllowed > truncationMarker.Length, $"Truncation marker \"{truncationMarker}\" is too long for maximum length {maximumLengthAllowed}.");
        ...
        public static T ParseAsEnum<T>(this int value) // where T : enum
        {
            ...
            Check.Require(Enum.IsDefined(typeof(T), enumValue), $"Enum does not have a corresponding value for integer. Enum type: {typeof(T).FullName}, integer value: {value}.");
            return enumValue;
        }
