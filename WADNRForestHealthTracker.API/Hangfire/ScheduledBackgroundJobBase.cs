...
public abstract class ScheduledBackgroundJobBase<T>
{
    ...
    protected readonly WADNRForestHealthTrackerDbContext DbContext;
    protected readonly WADNRForestHealthTrackerConfiguration WADNRForestHealthTrackerConfiguration;
    ...
    protected ScheduledBackgroundJobBase(string jobName, ILogger<T> logger, IWebHostEnvironment webHostEnvironment, WADNRForestHealthTrackerDbContext dbContext, IOptions<WADNRForestHealthTrackerConfiguration> ltInfoConfiguration, SitkaSmtpClientService sitkaSmtpClient)
    {
        _jobName = jobName;
        Logger = logger;
        _webHostEnvironment = webHostEnvironment;
        DbContext = dbContext;
        WADNRForestHealthTrackerConfiguration = ltInfoConfiguration.Value;
    ...
    public void RunJob(IJobCancellationToken token)
    {
        lock (ScheduledBackgroundJobLock)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Subject = $"WADNRForestHealthTracker Hangfire Job Failed: Job {_jobName}",
                    Body = $"Details: <br /><br />{ex.Message}",
                    IsBodyHtml = true
                };

                mailMessage.To.Add(new MailAddress(WADNRForestHealthTrackerConfiguration.SitkaEmailRedirect));
