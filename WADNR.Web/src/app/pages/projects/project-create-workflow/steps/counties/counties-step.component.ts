...
export class CountiesStepComponent extends WorkflowStepBase implements OnInit {
    ...
    ngOnInit(): void {
        ...
        const stepData$ = this._projectID$.pipe(
            switchMap((id) => {
                ...
                return this.projectService.getCountiesStepProject(id).pipe(
                    catchError(() => {
    ...
    onSave(navigate: boolean): void {
        ...
        this.saveStep(
            (projectID) => this.projectService.saveCountiesStepProject(projectID, request),
            "Counties saved successfully.",
            "Failed to save counties.",
            navigate
        );
    }
}
