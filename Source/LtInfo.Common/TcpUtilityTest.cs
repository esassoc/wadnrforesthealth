...
", hostThatSupportsHttp));
            Assert.That(result, Is.StringMatching("HTTP/1.1 [23][0-9][0-9] [A-z0-9 ]+"), "If the TCP socket thingy is working it should be able to make a sample connection such as for an HTTP GET request and get back a 200 or 300 level response");
        }
    }
}
