...
export interface AgreementProjectsModalData {
    agreementID: number;
    currentProjects: ProjectLookupItem[];
}

@Component({
    selector: "agreement-projects-modal",
    standalone: true,
        ...
        <div class="modal">
            ...
            <div class="modal-body" [loadingSpinner]="{ isLoading: isLoading(), loadingHeight: 200 }">
                ...
                @if (!isLoading()) {
                    ...
                    <label class="field-label">Associated Projects</label>
                    @if (selectedItems.length > 0) {
                        ...
                        <p class="system-text">No projects associated.</p>
                    }
                }
            </div>
...
export class AgreementProjectsModalComponent extends BaseModal implements OnInit, OnDestroy {
    public ref: DialogRef<AgreementProjectsModalData, boolean> = inject(DialogRef);
    ...
    ngOnInit(): void {
        ...
        this.selectedItems = [...data.currentProjects];
        ...
        this.projectService.listLookupProject().subscribe({
            next: (items) => {
                this.allItems = items;
                this.updateAvailableOptions();
    ...
    save(): void {
        ...
        this.agreementService.updateProjectsAgreement(agreementID, request).subscribe({
            next: () => {
                this.isSubmitting.set(false);
                this.ref.close(true);
            },
            error: () => {
                this.addLocalAlert("Failed to update projects.", AlertContext.Danger);
                this.isSubmitting.set(false);
            },
        });
    }
