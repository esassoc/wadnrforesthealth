...
export class PriorityLandscapesStepComponent extends WorkflowStepBase implements OnInit {
    ...
    ngOnInit(): void {
        ...
        const stepData$ = this._projectID$.pipe(
            switchMap((id) => {
                ...
                return this.projectService.getPriorityLandscapesStepProject(id).pipe(
                    catchError(() => {
    ...
    onSave(navigate: boolean): void {
        ...
        this.saveStep(
            (projectID) => this.projectService.savePriorityLandscapesStepProject(projectID, request),
