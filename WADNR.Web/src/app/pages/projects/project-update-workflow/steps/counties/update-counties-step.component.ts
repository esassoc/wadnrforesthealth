...
@Component({
    ...
    templateUrl: "./update-counties-step.component.html",
    styleUrls: ["./update-counties-step.component.scss"],
})
export class UpdateCountiesStepComponent extends UpdateWorkflowStepBase implements OnInit {
    readonly nextStep = "treatments";
    readonly stepKey = "Counties";

    // WMS layer configuration
    private readonly layerName = "WADNRForestHealth:County";
    private readonly idField = "CountyID";
    private readonly nameField = "CountyName";
    private readonly selectedStyle = "county_yellow";
    ...
    ngOnInit(): void {
        ...
        const stepData$ = this.stepRefresh$.pipe(
            switchMap((id) => {
                ...
                return this.projectService.getUpdateCountiesStepProject(id).pipe(
                    catchError(() => {
                        this.alertService.pushAlert(new Alert("Failed to load counties data.", AlertContext.Danger, true));
                        return of(null);
                    })
                );
            }),
    ...
    handleMapLoad(event: WADNRMapInitEvent): void {
        ...
        this.layerControl.addOverlay(baseLayer, "Counties", "Geographic Layers");
    ...
    private updateSelectedLayer(): void {
        ...
        if (this.layerControl) {
            this.layerControl.addOverlay(this.selectedLayer, "Selected Counties", "Geographic Layers");
        }
    ...
    onSave(navigate: boolean): void {
        ...
        if (selectedIDs.length === 0 && !explanation) {
            this.alertService.pushAlert(new Alert("Please select at least one county or provide an explanation in the Notes field.", AlertContext.Warning, true));
            return;
        }
        ...
        this.saveStep((projectID) => this.projectService.saveUpdateCountiesStepProject(projectID, request), "Counties saved successfully.", "Failed to save counties.", navigate);
    }
}
